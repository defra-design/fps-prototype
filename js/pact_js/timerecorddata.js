...
document.getElementById("moveBtn").addEventListener("click", () => {
  ...
  document
    ...
    .addEventListener("scroll", alignHoursBox);
  // end here
});
...
function addOrUpdateRow(data, existingRow) {
  ...
  if (existingRow) {
    // EDIT MODE
    existingId = editingRow.cells[0].querySelector(
      'input[type="checkbox"].timerecordCheckbox',
    ).dataset.id;
    tr = existingRow;
  }
...
function applyPaginationLiveRecords() {
  ...
  const filtered = rows.filter((row) => {
    ...
    const matchesWG =
      ...
      wgSearch === "" ||
      wgSearch === "--select--" ||
      workGroup.toLowerCase().includes(wgSearch.toLowerCase());
...
function importRows(rows) {
  ...
  // tmprow.remove(); 
  //Before importing new data, need to remove existing data except "No record found" row
...
function addData() {
  ...
  addRow({
    WorkGroup: document.getElementById("WorkGroup").value,
    ID: pactStaffID,
    PACTStaff: document.getElementById("dpName").value.split('|')[1],
    TimeCode: document.getElementById("TimeCode").value,
    ParentProject: document.getElementById("ParentProject").value,
    Period: document.getElementById("Period").value,
    Hours: document.getElementById("Hours").value,
...
const headersSelected = document.querySelectorAll(
  "#selectedTable th[data-column]",
);
